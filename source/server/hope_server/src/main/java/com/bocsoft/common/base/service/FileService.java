...
package com.bocsoft.common.base.service;
...
import com.bocsoft.common.kit.ExcelKit;
...
//	}
	public FileUploaded saveFile(UploadFile uploadFile, String objectId,String docType) {
		FileUploaded entity=this.createFileUploaded(uploadFile,objectId,docType);
		if(entity==null) {
			return null;
		}
		entity.save();
...
//	}
		public FileUploaded saveFile(UploadFile uf,String docType) {
		return this.saveFile(uf, null,docType);
	}
	...
	public List<FileUploaded> saveFiles(List<UploadFile> list,String docType) {
		return this.saveFiles(list,null,docType);
	}
...
//	}
	...
	public List<FileUploaded> saveFiles(List<UploadFile> list, String objectId,String docType) {
		List<FileUploaded> results = new ArrayList<FileUploaded>();
		List<FileUploaded> modelList=new ArrayList<>();
		for (UploadFile uf : list) {
			FileUploaded entity=createFileUploaded(uf, objectId,docType);
			if(entity!=null){
...
//	}
		...
		return Db.tx(new IAtom() {
			...
			public boolean run() throws SQLException {
				try {
					...
					Object[][] objs=new Object[list.size()][list.get(0).length+2];
					//Object[][] objs=new Object[list.size()][list.get(0).length];
					for(int i=0;i<list.size();i++){
						for(int j=0;j<list.get(0).length;j++){
							if(j==0){
								objs[i][0]="N";
								objs[i][1]=fileUploaded.getId();
								System.out.println("id的值为"+fileUploaded.getId());
								objs[i][j+2]=list.get(i)[j];
							}else{
								objs[i][j+2]=list.get(i)[j];
							}
						}
					...
					System.out.println("objs0数组长度"+Arrays.toString(objs[0]));
					//批量导入
	 ...
	 * @return
	 */
	private FileUploaded createFileUploaded(UploadFile uploadFile, String objectId,String docType){
		if(uploadFile==null) {
			return null;
		}
		// 获取已上传的文件
...
//			date = "";
		...
		fileUpload.setUrl(rename);
		fileUpload.setOperId(objectId);
		fileUpload.setInfoType(docType);
		return fileUpload;
	}
