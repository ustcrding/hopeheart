 ...
 * @returns
 */
function initGrid(options, events, initGridArgs, uploadNo) {
    var gridArgs_ = null;
...
//		$('#'+tableOptions.searchForm).removeAttr('action');
        ...
        $(ajaxBtn).click(function () {
            // var checkStatus = table.checkStatus(options.id); // test即为基础参数id对应的值
            var checkStatus = uploadNo; // test即为基础参数id对应的值
            // if(typeof checkStatus!= 'undefined'){
            layer.confirm('确认审核要素', {icon: 1, title: '提示'}, function (uploadNo) {
                // var ids = [];
                // for (var i = 0; i < checkStatus.data.length; i++) {
                // 	ids[i] = checkStatus.data[i][gridArgs_.dataId];
                // }
                // 向服务端发送ajax指令
                jQuery.post(gridArgs_.ajaxUrl + '?uploadNo=' + document.getElementById("uploadNo").value,
                    param(uploadNo), function (feedback) {
                        if (feedback.success) {
                            refreshData();
                            layer.alert(feedback.msg, {icon: 1});
                        } else {
                            layer.alert(feedback.msg, {icon: 2});
                        }
                    });
                layer.close(uploadNo);
            });
            // }else{
            // 	layer.alert('请选择行',{icon: 7});
            // }
