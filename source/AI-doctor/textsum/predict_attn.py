...
from headline import MediumConfig
from 文本摘要.seq2seq import data_utils as my_data_util

config = MediumConfig()  # new Large Config,
...
def decode():
    ...
    with tf.Session() as sess:
        ...
        while sentence:
            ...
            print(sentence)
            sentence = list(sentence[i] for i in range(0, len(sentence)))
            sentence = ' '.join(s for s in sentence)
            print("分词后结果： {}".format(sentence))
            ...
            _, _, output_logits_batch, attn_matrix = model.step(sess, encoder_inputs, decoder_inputs, target_weights,
                                                                bucket_id, True)
